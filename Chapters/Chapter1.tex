...
a triplet of real numbers can be represented by a point in 3D space, $\R^3$
...
Let us consider vector $\uv = \colvec{2}{3}{1}$. What is $2\cdot \uv$? We can calculate it as follows:
\[
...
\draw[->](-2,-2)--(1,2) node[anchor=west]{$\uv = \colvec{2}{3}{4}$};
\draw[dashed](1,-2)--(1,2);
\draw (1,0) node[anchor=west]{4};
\draw (-0.5,-2) node[anchor=north]{3};
\end{tikzpicture}
\end{center}
\end{example}
\begin{definition}
...
\end{lemma}
\begin{remark}
It is easy to see that Cauchy - Schwartz inequality is correct also for zero vectors.	
\end{remark}